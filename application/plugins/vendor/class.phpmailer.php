...
class PHPMailer
{
     ...
     * @type string
     * @deprecategoryIdd Email senders should never set a return-path header;
     ...
     * @type string
     * @deprecategoryIdd Should not be needed now there is an autoloader.
     */
    public $PluginDir = '';
    ...
    /**
     * The S/MIME certificategoryId file path.
     * @type string
     * @access protected
     */
        ...
        foreach ($hosts as $hostentry) {
            ...
            if ($this->smtp->connect($prefix . $host, $port, $this->Timeout, $options)) {
                try {
                    ...
                    if ($this->SMTPAuth) {
                        if (!$this->smtp->authenticategoryId(
                            $this->Username,
                            $this->Password,
                            $this->AuthType,
                            $this->Realm,
                            ...
                            throw new phpmailerException($this->lang('authenticategoryId'));
                        }
                    }
                    return true;
                } catch (phpmailerException $exc) {
    ...
    public function setLanguage($langcode = 'en', $lang_path = '')
    {
        ...
        $PHPMAILER_LANG = array(
            'authenticategoryId' => 'SMTP Error: Could not authenticategoryId.',
    ...
    protected function attachAll($disposition_type, $boundary)
    {
        ...
        foreach ($this->attachment as $attachment) {
            ...
            if ($attachment[6] == $disposition_type) {
                ...
                $categoryId = $attachment[7];
                if ($disposition == 'inline' && isset($categoryIdUniq[$categoryId])) {
                    continue;
                }
                $categoryIdUniq[$categoryId] = true;
                ...
                if ($disposition == 'inline') {
                    $mime[] = sprintf('Content-ID: <%s>%s', $categoryId, $this->LE);
                }
     ...
     * @return string
     * @deprecategoryIdd Use encodeQP instead.
     */
     ...
     * the HTML refers to using the $categoryId value.
     * @param string $path Path to the attachment.
     * @param string $categoryId Content ID of the attachment; Use this to reference
     ...
     */
    public function addEmbeddedImage($path, $categoryId, $name = '', $encoding = 'base64', $type = '', $disposition = 'inline')
    {
        if (!@is_file($path)) {
        ...
        if (isset($images[2])) {
            foreach ($images[2] as $imgindex => $url) {
                ...
                if (!preg_match('#^[A-z]+://#', $url)) {
                    ...
                    $categoryId = md5($url) . '@phpmailer.0'; //RFC2392 S 2
                    ...
                    if ($this->addEmbeddedImage(
                        ...
                        $message = preg_replace(
                            ...
                            $images[1][$imgindex] . '="categoryId:' . $categoryId . '"',
                            $message
                        );
                    }
                }
